import Image from 'next/image';

const Navbar = () => {
    return (
        <div className='flex items-center justify-between p-4'>
            {/* Search Bar */}
            <div className='hidden md:flex items-center gap-2 text-xs rounded-full ring-[1.5px] ring-gray-300 px-2  bg-transparent'>
                <Image alt='search' src="/search.png" width={14} height={14}></Image>
                <input className='w=[200px] p-2 bg-transparent outline-none' type="text" placeholder='Search....'/>
            </div>


            {/* Icons and User */}
            <div className='flex items-center gap-6 justify-end w-full'>
                <div className='bg-white rounded-full w-7 flex items-center justify-center cursor-pointer'>
                    <Image alt='message' src="/message.png"   width={20} height={20}></Image>
                </div>
                <div className='bg-white rounded-full w-7 flex items-center justify-center cursor-pointer relative'>
                    <Image alt='announcement' src="/announcement.png" width={20} height={20} />
                    <div className='absolute -top-3 -right-3 w-5 h-5 flex items-center justify-center bg-purple-500  rounded-full text-white text-xs'>
                        1
                    </div>
                </div>
                <div className='flex flex-col'>
                    <span className='text-xs leading-3 font-medium'>John Doe</span>
                    <span className='text-[10px] text-gray-500 text-right'>Admin</span>
                    
                </div>
                <Image className='rounded-full' alt='user' src="/avatar.png" width={36} height={36}/>
            </div>
        </div>
    );
};

export default Navbar;